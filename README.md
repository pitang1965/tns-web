...
- **車中泊スポット情報**: トイレ・入浴施設、コンビニまでの距離、安全性・静けさの情報を提供。車高（高さ制限）での絞り込みにも対応し、背の高い車でも入れるスポットを探せます。あなたのおすすめスポットも募集中！
...
- [shadcn/ui](https://ui.shadcn.com/) - Radix UI と Tailwind CSS をベースにしたアクセシブルな UI コンポーネント
- [Tailwind CSS](https://tailwindcss.com/) - ユーティリティファーストの CSS フレームワーク (v4)
- [Lucide React](https://lucide.dev/) - アイコンライブラリ
- [next-themes](https://github.com/pacocoursey/next-themes) - ダークモード／テーマ切り替え
- [Sonner](https://sonner.emilkowal.ski/) - トースト通知
- [canvas-confetti](https://github.com/catdad/canvas-confetti) - 紙吹雪などの演出エフェクト

### ドラッグ&ドロップ

- [dnd kit](https://dndkit.com/) - アクティビティの並べ替えに使用するドラッグ&ドロップツールキット

### PWA / パフォーマンス

- [@ducanh2912/next-pwa](https://github.com/DuCanhGH/next-pwa) - Next.js 向け PWA 対応（オフライン・インストール促進）
- [idb](https://github.com/jakearchibald/idb) - IndexedDB のラッパー（詳細ページのキャッシュによる高速化）
- [React Compiler](https://react.dev/learn/react-compiler) - 自動メモ化によるレンダリング最適化
...
- [Resend](https://resend.com/) - トランザクショナルメール送信サービス

### モニタリング・分析

- [Sentry](https://sentry.io/) - エラー追跡と監視
- [PostHog](https://posthog.com/) - プロダクト分析（トラフィック・機能利用・ファネル/定着）
...
- `SPOTS_API_KEY`（必須）:
  - `/api/v1/*`（モバイルアプリ向け公開API）を保護する固定APIキー。`src/proxy.ts` で検証される。
...
- `NEXT_PUBLIC_POSTHOG_KEY`:
  - PostHog の Project API Key。アクセス解析（トラフィック・機能利用・ファネル/定着）に使用される。
  - 本番環境でのみ初期化される（開発環境では計測しない）。
  - 例: `phc_XXXXXXXXXXXXXXXX`
- `NEXT_PUBLIC_POSTHOG_HOST`:
  - PostHog のイベント送信先ホスト。未設定時は `https://us.i.posthog.com`。
  - EU リージョンの場合は `https://eu.i.posthog.com`。
- `POSTHOG_PERSONAL_API_KEY` / `POSTHOG_PROJECT_ID` / `POSTHOG_API_HOST`（任意）:
  - 退会（アカウント削除）時に PostHog の person を削除するためのサーバー用設定。
  - 未設定の場合、person 削除はスキップされる（退会処理自体は成功する）。
  - `POSTHOG_API_HOST` の既定は `https://us.posthog.com`（EU は `https://eu.posthog.com`）。

### 広告

- `NEXT_PUBLIC_ADS_ENABLED`:
  - 広告の共通スイッチ。未設定または `false` で停止（既定）。`true` で再開。
  - 停止中は AdSense スクリプト、上部広告（nafuda バナーを含む）、詳細ページの広告枠、開発用プレースホルダー、地図・一覧・詳細の広告リンク（ポータブル電源セール等）を表示しません。
  - 広告リンクは共通スイッチと `NEXT_PUBLIC_SHOW_AD_LINK` の両方が `true` の場合だけ表示します。
  - 広告ID・スロット設定は削除不要です。再開は既存設定を残したまま `true` に変更し、再ビルド・デプロイしてください。
  - 本番の停止にも、この変更をデプロイする必要があります。既に開いているページやオフラインキャッシュは更新されるまで旧表示が残る場合があります。
...
- `RESEND_API_KEY`:
  - Resend の API キー。お問い合わせフォームやシステム通知メールの送信に使用。
  - Resend → API Keys → Create API Key で発行（Permission = Sending access、Domain = `over40web.club` に限定）。
  - 例: `re_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`
- `RESEND_FROM_EMAIL`:
  - メール送信時の送信者メールアドレス。Resend で検証済みの `over40web.club` ドメインのアドレスを指定。
...
2. **Resend 経由** → `tabi-support@over40web.club` から送信
...
このプロジェクトでは、使用している技術スタック（Next.js 16、React 19、Tailwind CSS、Jotai、MongoDB、Auth0 など）の最新ドキュメントを参照できるよう、プロジェクト固有の Context7 設定を行っています。
