...
- 気にした点: Vercel では `src/app/itineraries/generate/page.tsx` の `maxDuration = 300` で最大300秒待てるが、
  **Cloudflare のプロキシ経由では、無料プランだとサーバーの応答を約100秒しか待たない**（超えると 524）。
...
- 残り: 日数が多いなど重い条件での生成（100秒に近づかないか）。近づく場合は、裏で生成して完了を確認する方式や
  ストリーミングへの変更を検討する
...
- [ ] AI 生成を重い条件で試し、Cloudflare の約100秒の上限に近づかないか確かめる（普通の条件では約40秒）
