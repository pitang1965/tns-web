...
- [ ] 主要機能が Docker 内で動作する（トップ・地図・スポット詳細・旅程・管理画面）
- [ ] ログイン後の画面・旅程・AI 生成を含めた並列アクセスで異常がない
- [ ] Auth0 のログイン／コールバック／ログアウトが正常に戻る
- [ ] ISR と管理画面からの更新（`revalidateTag` / `updateTag`）が整合する
- [ ] Windows 再起動後に自動で復旧する
- [ ] Cloudflare Tunnel 経由の速度に問題がない
- [ ] 数日間の連続運転で落ちない
...
- [ ] 6. 障害復旧の練習（強制終了、Docker Desktop 再起動、回線断、Atlas 接続不可、Tunnel 再起動、旧イメージへの切り戻し）
- [ ] 7. 数日間の連続運転
...
- 判断: **現時点で、トンネル経由の速度は本番と同等以上**。PDX 経由に戻ることがないか、数日間の運転（手順7）の中で
