...
export const CampingSpotSchema = z.object({
  ...
  distanceToToilet: z
    .number()
    .min(0, 'トイレまでの距離は0以上で入力してください')
    .optional(),
  distanceToBath: z.number().min(0).optional(),
  distanceToConvenience: z.number().min(0).optional(),
  nearbyToiletCoordinates: z
    .tuple([
      z.number().min(-180).max(180), // longitude
      z.number().min(-90).max(90), // latitude
    ])
    .optional(),
  nearbyConvenienceCoordinates: z
    .tuple([
      z.number().min(-180).max(180), // longitude
      z.number().min(-90).max(90), // latitude
    ])
    .optional(),
  nearbyBathCoordinates: z
    .tuple([
      z.number().min(-180).max(180), // longitude
      z.number().min(-90).max(90), // latitude
    ])
...
export const CampingSpotCSVSchema = z.object({
  ...
  type: z
    .string()
    .refine((val) => CampingSpotTypeSchema.safeParse(val).success, {
  ...
  elevation: z
    ...
    .refine(
      (val) =>
        val === '' ||
        (!isNaN(Number(val)) && Number(val) >= -10 && Number(val) <= 3776),
      {
...
export const CampingSpotCSVJapaneseSchema = z.object({
  ...
  '標高(m)': z
    ...
    .optional()
    .default(''),
  'セキュリティ-ゲート有無(true/false)': z
    .string()
    .refine((val) => val === 'true' || val === 'false' || val === '', {
      message: 'ゲート有無はtrue/false/空欄で入力してください',
    })
    .optional()
    .default(''),
  'セキュリティ-照明十分(true/false)': z
    .string()
    .refine((val) => val === 'true' || val === 'false' || val === '', {
...
// For server-side operations (with MongoDB _id)
export const CampingSpotSubmissionWithIdSchema =
  CampingSpotSubmissionSchema.extend({
    _id: z.string(),
    createdAt: z.date(),
    updatedAt: z.date(),
  });
...
export type CampingSpotSubmissionWithId = z.infer<
  typeof CampingSpotSubmissionWithIdSchema
>;
export type CampingSpotCSV = z.infer<typeof CampingSpotCSVSchema>;
export type CampingSpotCSVJapanese = z.infer<
  typeof CampingSpotCSVJapaneseSchema
>;
...
export function csvRowToCampingSpot(csvRow: CampingSpotCSV): CampingSpot {
  return {
    ...
    distanceToToilet: csvRow.distanceToToilet
      ? Number(csvRow.distanceToToilet)
      : undefined,
    distanceToBath: csvRow.distanceToBath
      ? Number(csvRow.distanceToBath)
      : undefined,
    distanceToConvenience: csvRow.distanceToConvenience
      ? Number(csvRow.distanceToConvenience)
      : undefined,
    nearbyToiletCoordinates:
      csvRow.nearbyToiletLat && csvRow.nearbyToiletLng
        ? [Number(csvRow.nearbyToiletLng), Number(csvRow.nearbyToiletLat)]
        : undefined,
    nearbyConvenienceCoordinates:
      csvRow.nearbyConvenienceLat && csvRow.nearbyConvenienceLng
        ? [
            Number(csvRow.nearbyConvenienceLng),
            Number(csvRow.nearbyConvenienceLat),
          ]
        : undefined,
    nearbyBathCoordinates:
      csvRow.nearbyBathLat && csvRow.nearbyBathLng
        ? [Number(csvRow.nearbyBathLng), Number(csvRow.nearbyBathLat)]
        : undefined,
    ...
    pricing: {
      ...
      pricePerNight: csvRow.pricePerNight
        ? Number(csvRow.pricePerNight)
        : undefined,
...
export function csvJapaneseRowToCampingSpot(
  csvRow: CampingSpotCSVJapanese,
): CampingSpot {
  return {
    name: csvRow['名称'],
    ...
    distanceToToilet: csvRow['トイレまでの距離(m)']
      ? Number(csvRow['トイレまでの距離(m)'])
      : undefined,
    distanceToBath: csvRow['お風呂までの距離(m)']
      ? Number(csvRow['お風呂までの距離(m)'])
      : undefined,
    distanceToConvenience: csvRow['コンビニまでの距離(m)']
      ? Number(csvRow['コンビニまでの距離(m)'])
      : undefined,
    nearbyToiletCoordinates:
      csvRow['トイレ緯度'] && csvRow['トイレ経度']
        ? [Number(csvRow['トイレ経度']), Number(csvRow['トイレ緯度'])]
        : undefined,
    nearbyConvenienceCoordinates:
      csvRow['コンビニ緯度'] && csvRow['コンビニ経度']
        ? [Number(csvRow['コンビニ経度']), Number(csvRow['コンビニ緯度'])]
        : undefined,
    nearbyBathCoordinates:
      csvRow['お風呂緯度'] && csvRow['お風呂経度']
        ? [Number(csvRow['お風呂経度']), Number(csvRow['お風呂緯度'])]
        : undefined,
    ...
    nightNoise: {
      ...
      nearBusyRoad:
        csvRow['夜間騒音-交通量多い道路近く(true/false)'] === 'true',
