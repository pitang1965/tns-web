...
export const useCheckMissingFields = () => {
  ...
  const checkMissingFields = (data: ShachuHakuFormData): MissingFields => {
    ...
    const hasToiletLat =
      data.nearbyToiletLat && data.nearbyToiletLat.trim() !== '';
    const hasToiletLng =
      data.nearbyToiletLng && data.nearbyToiletLng.trim() !== '';
    if (!hasToiletLat || !hasToiletLng) {
      empty.push('トイレの緯度及び/又は経度');
    }

    const hasConvenienceLat =
      data.nearbyConvenienceLat && data.nearbyConvenienceLat.trim() !== '';
    const hasConvenienceLng =
      data.nearbyConvenienceLng && data.nearbyConvenienceLng.trim() !== '';
    ...
    if (!data.nightNoiseHasNoiseIssues)
      unchecked.push('夜間騒音問題有りでない');
    if (!data.nightNoiseNearBusyRoad)
      unchecked.push('交通量多い道路近くでない');
