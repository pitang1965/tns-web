...
type NearbyCampingSpotsListProps = {
...
export function NearbyCampingSpotsList({
  spots,
  onSelect,
  selectedSpotId,
}: NearbyCampingSpotsListProps) {
  if (spots.length === 0) {
    return (
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-shrink-0 flex items-center gap-1 text-primary font-semibold">
                      ...
                      <span className="text-sm">{formatDistance(spot.distance)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <div className="inline-flex items-center gap-1 px-2 py-1 bg-secondary rounded-md text-xs">
                      ...
                      {spot.pricing.isFree ? (
                        ...
                        <span>¥{spot.pricing.pricePerNight.toLocaleString()}/泊</span>
                      ) : (
                        <span>料金不明</span>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    {spot.distanceToToilet !== undefined && (
                      <span>トイレまで {formatDistance(spot.distanceToToilet)}</span>
                    )}
                    {spot.distanceToBath !== undefined && (
                      <span>入浴施設まで {formatDistance(spot.distanceToBath)}</span>
                    )}
                    {spot.distanceToConvenience !== undefined && (
                      <span>コンビニまで {formatDistance(spot.distanceToConvenience)}</span>
                    )}
                  </div>
