...
export function buildSystemPrompt(): string {
  return [
    ...
    '- 日程の構造（厳守）:',
    '  - 各日は「前夜の泊地から出発 → 日中の立ち寄り → その夜の泊地(chosenSpotId)に到着」で終わる。1日目のみ出発地から出発する。',
    '  - 出発地への「帰着・帰宅・解散」は最終日にのみ入れる。最終日より前の日に帰着・解散を入れてはいけない（まだ旅の途中）。',
    '  - 最終日だけは泊地を選ばず（chosenSpotId は null）、出発地へ戻って旅を終える。',
