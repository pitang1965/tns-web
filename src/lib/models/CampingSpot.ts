...
campingSpotSchema.pre('save', function (next) {
  ...
  const validateCoordinates = (coords: number[], fieldName: string) => {
    if (coords && coords.length > 0) {
      if (coords.length !== 2) {
        return new Error(
          `${fieldName} must have exactly 2 elements [longitude, latitude]`,
        );
      }
      if (
        coords[0] < -180 ||
        ...
        return new Error(
          `${fieldName} must be within valid ranges: longitude (-180 to 180), latitude (-90 to 90)`,
        );
      }
    }
    return null;
  ...
  const toiletError = validateCoordinates(
    this.nearbyToiletCoordinates || [],
    'nearbyToiletCoordinates',
  );
  if (toiletError) return next(toiletError);

  const convenienceError = validateCoordinates(
    this.nearbyConvenienceCoordinates || [],
    'nearbyConvenienceCoordinates',
  );
  if (convenienceError) return next(convenienceError);

  const bathError = validateCoordinates(
    this.nearbyBathCoordinates || [],
...
const CampingSpot =
  ...
  mongoose.model<ICampingSpot>('CampingSpot', campingSpotSchema);
